...
!-----------------------------------------------------------------------------
      ...
      use droplet_module, only: initialize_droplet_histograms,particle_seed,ngidx
      use mpi
...
!-----------------------------------------------------------------------
      ...
      logical, parameter :: use_truly_random_pert  =  .true.
...
!  parcel info:
      ...
      IF(iprcl.eq.1)THEN
        ...
        !
        ! The generated random number is independent of compilers and 1 ranks
        call date_and_time(VALUES=values)
        !To get repeatable sequence
        !particle_seed =  max(1, int(nparcelsInit * write_frac_droplet))
	...
	ngidx = 0  !Set the global particle ID counter to 0 at the beginning
	...
	zrange = maxz
        ...
        if ( mod(real(nparcelsInit),real(numprocs)) /= 0 ) then
           ...
           ! evenly between different 1 ranks
        ...
        do n = 1, iter
          ...
          if (prcl_droplet.eq.1) then
             ...
             pdata(i,prpidx) = transfer(ngidx,0.0)
             pdata(i,prprocidx) = transfer(myid,0.0)
...
! Random perturbations:
      ...
      ELSEIF( testcase.eq.1 .or. testcase.eq.2 .or. testcase.eq.6 .or. testcase.eq.10 .or. testcase.eq.11 .or. testcase.eq.15 )THEN
        ...
        ! 6,10,15: hurricane boundary layer (Bryan et al, 2017, BLM)
