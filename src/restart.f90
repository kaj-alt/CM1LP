  ...
  CONTAINS
      ...
      double precision, intent(in)    :: avgsfcu,avgsfcv,avgsfcs,avgsfct,avgsfcq
...
! metadata:
  ...
  IF( restart_format.eq.1 )THEN
    IF(myid.eq.0)THEN
      ...
      write(50) radtim
      write(50) prcltim
      !$acc update host(adt,acfl,dbldt,mass1,avgsfcu,avgsfcv,avgsfcs,avgsfct,avgsfcq) if_present
      write(50) adt
      write(50) acfl
      write(50) dbldt
      write(50) mass1
      ...
      if(iprcl.eq.1)then
        ...
        if(myid.eq.0)then
          if( restart_format.eq.1 )then
            ...
            ! write out the number of active droplets per 1 rank
            do i = 1, numprocs
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      integer :: nparcels_per_mpi(numprocs)       ! number of active droplets per 1 rank
...
!  parcels:
      ...
      if( myid.eq.0 )then
        if( restart_format.eq.1 )then
          ...
          ! Read in the number of active droplets for each 1 rank
          do i = 1, numprocs
