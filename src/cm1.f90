...
!-----------------------------------------------------------------------------
      ...
      double precision :: avgsfcu,avgsfcv,avgsfcs,avgsfct,avgsfcq
      logical :: misc_detail
      logical :: dosfcflx
      ...
      logical :: different_namelist=.FALSE.
      ...
      real, dimension(:,:), allocatable :: tsk,znt,rznt,zntmp,ust,stau,tst,qst,z0t,z0q,thflux,qvflux,  &
                                           cd,ch,cq,u1,v1,s1,t1,q1,xland,psfc,tlh,f2d,psmth,prate,ustt,ut,vt,st
...
!-----
      ...
      integer :: i,j,k,m,n,nn,fnum,frec
      real :: sum,tem0,tem
...
!----------------------------------------------------------------------
!  Check to see if the user has specified a different namelist input
!  file to use
!
      n = command_argument_count() 

      do i = 1, n
          call get_command_argument(number=i, value=arg, length=j)

          select case(adjustl(arg))
             case ('-n', '--namelist')
                  different_namelist=.TRUE.
              case ('-h', '--help')
                  print *,'cammand line options:'
                  print *,'-n, --namelist   set a different namelist input file (default: namelist.input)'
                  print *,'-h, --help       print command line options'
                  call exit(0) 
              case default
                  if(different_namelist) then
                      namelist_input = adjustl(arg)
                      different_namelist=.FALSE.
                  else
...
!---------------------------------------------------------------------
      ...
      !$acc update device(ib,ie,jb,je,kb,ke,ni,nj,nip1,njp1,nk,nkp1,nkm1,ngxy,ngz)
...
!  Now, allocate the mother lode, then call INIT3D
      ...
      v10e = 0.0
      allocate(   th10(ibl:iel,jbl:jel) )
      th10 = 0.0
      allocate(    q10(ibl:iel,jbl:jel) )
      q10 = 0.0
...
!----------------------------------------------------------------------
      ...
      allocate( dpten(ib:ie,jb:je,kb:ke,7) )
      dpten = 0.0
...
!----------------------------------------------------------------------
      ...
      call init3d(xh,rxh,uh,ruh,xf,rxf,uf,ruf,yh,vh,rvh,yf,vf,rvf,  &
                  ...
                  pta,pt3d,ptten,                                   &
                  pdata,cfb,cfa,cfc,d1,d2,pdt,lgbth,lgbph,rhs,      &
                  trans)
...
!----------------------------------------------------------------------
      ...
      IF(irst.eq.1)THEN
        ...
        call     read_restart(nstep,srec,sirec,urec,vrec,wrec,nrec,mrec,prec,      &
                              ...
                              avgsfcu,avgsfcv,avgsfcs,avgsfct,avgsfcq,             &
...
!  All done with initialization and setup.  A few more odds and ends ....
      ...
      if( adapt_dt.eq.1 )then
        ...
        if( .not. restarted )then
          if(myid.eq.0) print *,'cm1: before call to calccflquick: ',dbldt
          call calccflquick(dt,uh,vh,mh,u3d,v3d,w3d,reqc)
          call mpi_wait(reqc,mpi_status_ignore,ierr)
          if(myid.eq.0) print *,'cm1: cflmax: ',cflmax
...
!$acc              cd,ch,cq,tlh,u1,v1,s1,t1,q1,th10,q10,znt,zntmp,ust,hfx, &
...
122     format(1x,'cflmax,ksmax,dt,nsound:',2x,f6.4,2x,f6.4,2x,f9.4,2x,i3)
      ...
      call sfc_and_turb(getsfc,getpbl,nstep,dt,dosfcflx,cloudvar,qbudget, &
                   ...
                   tsk,thflux,qvflux,cd,ch,cq,u1,v1,s1,t1,q1,tlh,f2d,ustt,ut,vt,st,   &
                   dum1,dum2,dum3,dum4,dum5,dum6,dum7,dum8,dum9,     &
                   divx,rho,rr,rf,prs,                               &
                   t11,t12,t13,t22,t23,t33,                          &
                   m11,m12,m13,m22,m23,m33,                          &
      ...
      if( dostat )then
        ...
        ! JS: with the new 1 implementation, we can:
        !     1) use MPI_GATHER to collect the droplet information from different
        !        1 ranks first and only the root rank calculates the diagnostic
        !     2) let all the 1 ranks call the diagnostic and use MPI_REDUCE
        !        to collect the results from different 1 ranks and
        ...
        if ( iprcl.eq.1 .and. prcl_droplet.eq.1 ) then
           call droplet_diag(rtime,nrec,zh,zf,pdata,pdata_locind,dpten,wa,qa,tha,th0,pi0,ppi,rho)
        end if
    ...
    IF( dodomaindiag )THEN
      if( dotdwrite )then
        ...
        call   domaindiag(nstep,mtime,nwritet,trecs,trecw,qname,qunit,dt,dosfcflx, &
                   ...
                   rain,hfx,qfx,u10,v10,s10,th10,q10,t2,q2,th2,mznt,           &
      ...
      !cccccccccccccccccccccccccccccccccccccccccccccccccccccccc!
        ...
        if(timestats.eq.2)then
          steptime2=time_sound+time_poiss+time_buoyan+time_turb+            &
                    ...
                    time_misc16+time_misc17+time_misc18+time_misc19+time_misc20+ &
...
157       format('    timing for time step ',i12,':',f12.4,' s')
      ...
      !print *,'mtime: ',mtime
      if(nstep .eq. 10) then 
        !stop 'cm1: at the end of timeloop'
      endif
...
!----------------------------------------------------------------------
    ...
    IF(timestats.ge.1)THEN
      ...
      IF(.not.procfiles)THEN
        sum = 0.0
        call MPI_REDUCE(time_H2D ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_H2D = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_D2H ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_D2H = sum/float(numprocs)

        sum = 0.0
        call MPI_REDUCE(time_phys_H2D ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_phys_H2D = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_phys_D2H ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_phys_D2H = sum/float(numprocs)
        ...
        sum = 0.0
        call MPI_REDUCE(time_bcs     ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_bcs = sum/float(numprocs)

        sum = 0.0
        call MPI_REDUCE(time_bcs2     ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_bcs2 = sum/float(numprocs)

        sum = 0.0
        call MPI_REDUCE(time_bcst     ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_bcst = sum/float(numprocs)

        sum = 0.0
        call MPI_REDUCE(time_nvtx   ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_nvtx = sum/float(numprocs)
        ...
        sum = 0.0
        call MPI_REDUCE(time_misc01    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc01 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc02    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc02 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc03    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc03 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc04    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc04 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc05    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc05 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc06    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc06 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc07    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc07 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc08    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc08 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc09    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc09 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc10    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc10 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc11    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc11 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc12    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc12 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc13    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc13 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc14    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc14 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc15    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc15 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc16    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc16 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc17    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc17 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc18    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc18 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc19    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc19 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc20    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc20 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc21    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc21 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc22    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc22 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_misc23    ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_misc23 = sum/float(numprocs)
        ...
        sum = 0.0
        call MPI_REDUCE(time_dropC1 ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_dropC1 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_dropC2 ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_dropC2 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_dropC3 ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_dropC3 = sum/float(numprocs)
        sum = 0.0
        call MPI_REDUCE(time_dropC4 ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_dropC4 = sum/float(numprocs)

        sum = 0.0
        call MPI_REDUCE(time_parceli_reduce ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_parceli_reduce = sum/float(numprocs)
        ...
        sum = 0.0
        call MPI_REDUCE(time_dpc     ,sum,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        time_dpc = sum/float(numprocs)
      ENDIF
      ...
      if(dowr) write(outfile,*) 'H2D:',time_H2D
      if(dowr) write(outfile,*) 'D2H:',time_D2H
      if(dowr) write(outfile,*)
      time_solve=0.01*time_solve
      if(time_solve.lt.0.0001) time_solve=1.

    IF(dowr)THEN
      write(outfile,100) 'phys-H2D',time_phys_H2D,time_phys_H2D/time_solve
      write(outfile,100) 'phys-D2H',time_phys_D2H,time_phys_D2H/time_solve
      ...
      write(outfile,100) 'bcs2    ',time_bcs2,time_bcs2/time_solve
      write(outfile,100) 'bcst    ',time_bcst,time_bcst/time_solve
      write(outfile,100) 'nvtx   ',time_nvtx,time_nvtx/time_solve
      ...
      misc_detail = .false.
      if(.not.misc_detail) then  
        time_misc = time_misc  +time_misc01+time_misc02+time_misc03+time_misc04+time_misc05+ &
                    time_misc06+time_misc07+time_misc08+time_misc09+time_misc10+time_misc11+ &
                    time_misc12+time_misc13+time_misc14+time_misc15+time_misc16+time_misc17+ &
                    time_misc18+time_misc19+time_misc20+time_misc21+time_misc22+time_misc23
        write(outfile,100) 'misc    ',time_misc,time_misc/time_solve
      else
        write(outfile,100) 'misc    ',time_misc,time_misc/time_solve
        write(outfile,100) 'misc01  ',time_misc01,time_misc01/time_solve
        write(outfile,100) 'misc02  ',time_misc02,time_misc02/time_solve
        write(outfile,100) 'misc03  ',time_misc03,time_misc03/time_solve
        write(outfile,100) 'misc04  ',time_misc04,time_misc04/time_solve
        write(outfile,100) 'misc05  ',time_misc05,time_misc05/time_solve
        write(outfile,100) 'misc06  ',time_misc06,time_misc06/time_solve
        write(outfile,100) 'misc07  ',time_misc07,time_misc07/time_solve
        write(outfile,100) 'misc08  ',time_misc08,time_misc08/time_solve
        write(outfile,100) 'misc09  ',time_misc09,time_misc09/time_solve
        write(outfile,100) 'misc10  ',time_misc10,time_misc10/time_solve
        write(outfile,100) 'misc11  ',time_misc11,time_misc11/time_solve
        write(outfile,100) 'misc12  ',time_misc12,time_misc12/time_solve
        write(outfile,100) 'misc13  ',time_misc13,time_misc13/time_solve
        write(outfile,100) 'misc14  ',time_misc14,time_misc14/time_solve
        write(outfile,100) 'misc15  ',time_misc15,time_misc15/time_solve
        write(outfile,100) 'misc16  ',time_misc16,time_misc16/time_solve
        write(outfile,100) 'misc17  ',time_misc17,time_misc17/time_solve
        write(outfile,100) 'misc18  ',time_misc18,time_misc18/time_solve
        write(outfile,100) 'misc19  ',time_misc19,time_misc19/time_solve
        write(outfile,100) 'misc20  ',time_misc20,time_misc20/time_solve
        write(outfile,100) 'misc21  ',time_misc21,time_misc21/time_solve
        write(outfile,100) 'misc22  ',time_misc22,time_misc22/time_solve
        write(outfile,100) 'misc23  ',time_misc23,time_misc23/time_solve
      endif
      ...
      write(outfile,100) 'dropC1  ',time_dropC1,time_dropC1/time_solve
