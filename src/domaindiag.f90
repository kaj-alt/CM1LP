...
!-------------------------------------------------------------------------------
    ...
    use input
    use constants
    use cm1libs , only : eslf,rslf,rsif
    use getcape_module
    ...
    real, intent(in), dimension(ibl:iel,jbl:jel) :: hfx,qfx,u10,v10,s10,th10,q10,t2,q2,th2,mznt
...
151     format(i6.6)
  ...
  DO wloop = 1 , wloopmax
    ...
    if( imove.eq.1 )then
      ...
      call write2d(savg2d,trecs,trecw,vargrid,nvar,varname,vardesc,varunit)
      if( myid.eq.0 ) print *,'  Metric: u component of 10m wind speed (m/s)= ',savg2d
    ...
    if( imove.eq.1 )then
      ...
      call write2d(savg2d,trecs,trecw,vargrid,nvar,varname,vardesc,varunit)
      if( myid.eq.0 ) print *,'  Metric: v component of 10m wind speed (m/s)= ',savg2d
    ...
    if( imove.eq.1 )then
      ...
      call write2d(savg2d,trecs,trecw,vargrid,nvar,varname,vardesc,varunit)
      if( myid.eq.0 ) print *,'  Metric: horiz wind speed at 10m (m/s)= ',savg2d
      ...
      varname(nvar) = 'th10'
    if( imove.eq.1 )then
      vardesc(nvar) = 'potential temp. at 10m'
    else
      vardesc(nvar) = 'potential temp. at 10m'
    endif
      varunit(nvar) = 'K'
      varlvls(nvar) = 0
      vargrid(nvar) = '2'
      call getavg2d( th10  ,savg2d)
      call write2d(savg2d,trecs,trecw,vargrid,nvar,varname,vardesc,varunit)
      if( myid.eq.0 ) print *,'  Metric: Potential temperature at 10m (K)= ',savg2d
      ...
      varname(nvar) = 'q10'
    if( imove.eq.1 )then
      vardesc(nvar) = 'water vapor mixing ratio at 10m'
    else
      vardesc(nvar) = 'water vapor mixing ratio at 10m'
    endif
      varunit(nvar) = 'kg/kg'
      varlvls(nvar) = 0
      vargrid(nvar) = '2'
      call getavg2d( q10  ,savg2d)
      call write2d(savg2d,trecs,trecw,vargrid,nvar,varname,vardesc,varunit)
      if( myid.eq.0 ) print *,'  Metric: Water vapor mixing ratio at 10m (K)= ',savg2d
    ...
    ziflag:  &
    IF( ( testcase.ge.1 .and. testcase.le.7 ) .or. testcase.eq.9 .or. testcase.eq.10 .or. testcase.eq.11 .or. testcase.eq.15 )THEN
      ...
      call getavg2d( dum1(ib,jb,1) ,savg2d)
      call write2d(savg2d,trecs,trecw,vargrid,nvar,varname,vardesc,varunit)
      if( myid.eq.0 ) print *,'  Metric: boundary-layer depth (m)= ',savg2d
      zibar = savg2d
      ...
      varlvls(nvar) = 0
      vargrid(nvar) = '2'
      if( myid.eq.0 ) print *,'  Metric: boundary-layer depth min (m)= ',zimin
      savg2d = zimin
      if( myid.eq.0 ) print *,'  Metric: boundary-layer depth min (m)= ',zimin
      call write2d(savg2d,trecs,trecw,vargrid,nvar,varname,vardesc,varunit)
      if( myid.eq.0 ) print *,'  Metric: boundary-layer depth min (m)= ',savg2d
      ...
      savg2d = zimax
      call write2d(savg2d,trecs,trecw,vargrid,nvar,varname,vardesc,varunit)
      if( myid.eq.0 ) print *,'  Metric: boundary-layer depth max (m)= ',savg2d
    ...
    ENDIF  ziflag
    !stop 'domaindiag: after boundary-layer depth diagnostics'
    ...
    IF( imoist.eq.1 )THEN
      ...
      nvar = nvar+1
      varname(nvar) = 'prate'
      vardesc(nvar) = 'domain-averaged surface precipitation rate'
...
!!!        call write3d(savg,trecs,trecw,vargrid,nvar,varname,vardesc,varunit)
    ...
    ENDDO  hloop
      ...
      IF( imoist.eq.1 )THEN
        ...
        nvar = nvar+1
        varname(nvar) = 'rh2'
        vardesc(nvar) = 'relative humidity with eslf'
        varunit(nvar) = 'nondimensional'
        ...
        do j=1,nj
        do i=1,ni
          !dum1(i,j,k) = rho(i,j,k)*(q3d(i,j,k,nqv)/(1 + q3d(i,j,k,nqv)))*rv*(th(i,j,k)*(pi0(i,j,k)+pp3d(i,j,k)))/eslf(prs(i,j,k),th(i,j,k)*(pi0(i,j,k)+pp3d(i,j,k)))
          dum1(i,j,k) = rho(i,j,k)*q3d(i,j,k,nqv)*rv*(th(i,j,k)*(pi0(i,j,k)+pp3d(i,j,k)))/eslf(prs(i,j,k),th(i,j,k)*(pi0(i,j,k)+pp3d(i,j,k)))
        enddo
        enddo
        enddo
        !$acc end parallel
        ...
        nvar = nvar+1
        varname(nvar) = 'rh3'
        vardesc(nvar) = 'relative humidity with eslf and ideal gas rho'
        varunit(nvar) = 'nondimensional'
        ...
        do j=1,nj
        do i=1,ni
          dum1(i,j,k) = (prs(i,j,k)/(th(i,j,k)*(pi0(i,j,k)+pp3d(i,j,k))*rd*(1+0.61*q3d(i,j,k,nqv))))*(q3d(i,j,k,nqv)/(1 + q3d(i,j,k,nqv)))*rv*(th(i,j,k)*(pi0(i,j,k)+pp3d(i,j,k)))/eslf(prs(i,j,k),th(i,j,k)*(pi0(i,j,k)+pp3d(i,j,k)))
        enddo
        enddo
        enddo
        !$acc end parallel
   ...
   !..................................
    ...
    ENDIF  turbcheck
      ...
      !c-c-c-c-c-c-c-c-c-c
        ...
        do j=1,nj
        do i=1,ni
          dum2(i,j,k) = 0.5*(dum1(i,j,k-1)+dum1(i,j,k))
        enddo
        enddo
        enddo
        !$acc end parallel
...
!!!      ENDIF
    ...
    !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      !c-c-c-c-c-c-c-c-c-c
        ...
        call getmax(w3d(ib,jb,kb),savg,nkp1)
        call write3d(savg,trecs,trecw,vargrid,nvar,varname,vardesc,varunit)
      ...
      !c-c-c-c-c-c-c-c-c-c
        ...
        call getmin(w3d(ib,jb,kb),savg,nkp1)
        call write3d(savg,trecs,trecw,vargrid,nvar,varname,vardesc,varunit)
    ...
    IF( cm1setup.eq.1 .and. iusetke )THEN
        ...
        !$acc loop gang vector reduction(+:savg2d)
        do k=2,nk
        ...
        vargrid(nvar) = '2'
        savg2d = 0.0
        !$acc parallel loop gang vector default(present) reduction(+:savg2d)
        do k=2,nk
    ...
    IF( output_format.eq.1 )THEN
      ...
      ! write descriptor files:
        do nfile=1,2
          ...
          enddo
          if( nfile.eq.1 )then
            string = 'cm1out_diag_00%y4_s.dat'
          elseif( nfile.eq.2 )then
            string = 'cm1out_diag_00%y4_w.dat'
          endif
          write(66,101) string
          write(66,102)
          if( nfile.eq.1 )then
          ...
          if( nfile.eq.1 )then
            write(66,107) nk
            !$acc update host(zh(1,1,1:nk))
            do k=1,nk
              write(66,217) 0.001*zh(1,1,k)
            enddo
          elseif( nfile.eq.2 )then
            write(66,107) nk+1
            !$acc update host(zf(1,1,1:nk))
            do k=1,nk+1
...
!-------------------------------------------------------------------
  ...
  ENDDO  bigwloop
    ...
    if( myid.eq.0 ) print *
    !stop 'domaindiag: at the end of the subroutine'
    ...
    !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      double precision :: ubart
      double precision, dimension(nk) :: ubar,vbar
      double precision :: vbart,vfrt,vfdt,ufst,vfst,ufrt,ufdt

      !$acc data create(ubar,vbar)
      weps = 100.0*epsilon
  ...
  !----------------------
      ...
      do k=2,nk
        ufrt=0.0d0
        ufdt=0.0d0
        !$acc loop vector collapse(2) reduction(+:ufrt,ufdt)
        do j=1,nj
        do i=1,ni
          wbar = 0.5*(w3d(i,j,k)+w3d(i-1,j,k))
          ufrt = ufrt + wbar*(dum4(i,j,k)-ubar(k))
          ufdt = ufdt + wbar*(dum3(i,j,k)-dum4(i,j,k))
        !buh5
        enddo
        enddo
        ufr(k) =ufrt
  ...
  !----------------------
      ...
      do k=2,nk
        vfrt = 0.0d0
        vfdt = 0.0d0
        !$acc loop vector collapse(2) reduction(+:vfrt,vfdt)
        do j=1,nj
        do i=1,ni
      ...
      if( cm1setup.ge.1 .or. ipbl.eq.2 )then
        if( sgsmodel.eq.5 .or. sgsmodel.eq.6 )then
          ...
          do k=1,nk+1
            ufst = 0.0d0
            vfst = 0.0d0
            !$acc loop vector collapse(2) reduction(+:ufst,vfst)
            do j=1,nj
            do i=1,ni
              ufst = ufst - m13(i,j,k)/(0.5*(rf(i,j,k)+rf(i-1,j,k)))
              vfst = vfst - m23(i,j,k)/(0.5*(rf(i,j,k)+rf(i,j-1,k)))
            enddo
            enddo
            ufs(k) = ufst
            vfs(k) = vfst
          ...
          do k=1,nk+1
            ...
            do j=1,nj
            do i=1,ni
              ufst = ufst- t13(i,j,k)/(0.5*(rf(i,j,k)+rf(i-1,j,k)))
              vfst = vfst- t23(i,j,k)/(0.5*(rf(i,j,k)+rf(i,j-1,k)))
            enddo
            enddo
            ufs(k) = ufst
            vfs(k) = vfst
    ...
    !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      double precision :: sbart,sfrt,sfst,sfst1,sfst2,sfdt
  ...
  !----------------------
    ...
    do k=2,nk
      sbart=0.d0
      !$acc loop vector collapse(2) reduction(+:sbart)
      do j=1,nj
      do i=1,ni
        sbart = sbart+dum3(i,j,k)
      enddo
      enddo
      sbar(k) = sbart
    enddo
  ...
  !----------------------
    ...
    do k=2,nk
      sfrt=0.0d0
      sfdt=0.0d0
      !$acc loop vector collapse(2) reduction(+:sfrt,sfdt)
      do j=1,nj
      do i=1,ni
    ...
    if( cm1setup.ge.1 .or. ipbl.eq.2 .or. cm1setup.eq.3 )then
      ...
      do k=2,nk
        sfst=0.0d0
        !$acc loop vector collapse(2) reduction(+:sfst)
        do j=1,nj
        do i=1,ni
          sfst = sfst - khv(i,j,k)*(s(i,j,k)-s(i,j,k-1))*rdz*mf(i,j,k)
        enddo
        enddo
        sfs(k) = sfst
      enddo
      ...
      ELSEIF( cm1setup.eq.3 )THEN
          ...
          elseif(bc_temp.eq.2)then
            ...
            do j=1,nj
            do i=1,ni
              sfst1 = sfst1 + (viscosity/pr_num)*ptc_bot
    ...
    real, intent(in), dimension(ib:ie,jb:je,kb:ke) :: s
    double precision, intent(out), dimension(nk+1) :: savg
    integer, intent(in), optional :: nkm
    !$acc declare present(s,savg,nkm)
...
!!!    k2 = nk
    ...
    do k=1,k2
      stmp = 0.0
      !$acc loop vector collapse(2) reduction(+:stmp)
      do j=1,nj
      do i=1,ni
        stmp = stmp+s(i,j,k)
      enddo
