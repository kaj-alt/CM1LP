...
!-----------------------------------------------------------
      ...
      DO J=jts,jte
        DO i=its,ite
           dz8w1d(I) = dz8w(i,j,kts)
           dz2w1d(I) = dz8w(i,j,kts+1)
           U1D(i) =U3D(i,j,kts)
           V1D(i) =V3D(i,j,kts)
           !2nd model level winds - for diags with high-res grids
           U1D2(i) =U3D(i,j,kts+1)
           V1D2(i) =V3D(i,j,kts+1)
           QV1D(i)=QV3D(i,j,kts)
           QC1D(i)=QC3D(i,j,kts)
           P1D(i) =P3D(i,j,kts)
           T1D(i) =T3D(i,j,kts)
           RHO1D(i)=RHO3D(i,j,kts)
           if (spp_pbl==1) then
               rstoch1D(i)=pattern_spp_pbl(i,j,kts)
           else
               rstoch1D(i)=0.0
           endif
        ENDDO
        ...
        IF (itimestep==1) THEN
           DO i=its,ite
              ...
              MOL(i,j)=0.     ! Tstar
              QSFC(i,j)=QV3D(i,j,kts)/(1.+QV3D(i,j,kts))
              qstar(i,j)=0.0
           ENDDO
        ENDIF
