  ...
  CONTAINS
      ...
      use input, only: ib,ie,jb,je,kb,ke,itb,ite,jtb,jte,ibnba,ienba,jbnba,jenba,kbnba,kenba, & 
          ...
          icloud_bl,spp_pbl,oml_gamma,grav_settling,initflag,cnstcd
      use constants
...
! Arrays and variables passed into solve
      ...
      double precision, intent(inout) :: avgsfcu,avgsfcv,avgsfcs,avgsfct,avgsfcq
      ...
      real, intent(inout), dimension(ib:ie,jb:je) :: tsk,znt,rznt,zntmp,ust,stau,tst,qst,z0t,z0q,thflux,qvflux,  &
                                                     cd,ch,cq,u1,v1,s1,t1,q1,psfc,tlh,ustt,ut,vt,st
      ...
      real, intent(inout), dimension(ibl:iel,jbl:jel) :: u10,v10,s10,th10,q10,hfx,qfx, &
                                      hpbl,wspd,phim,phih,psim,psih,psiq,gz1oz0,br,brcr, &
                                      CHS,CHS2,CQS2,CPMM,ZOL,MAVAIL,          &
                                      MOL,RMOL,REGIME,LH,FLHC,FLQC,QGH,       &
                                      CK,CKA,CDA,USTM,QSFC,T2,Q2,TH2,EMISS,THC,ALBD,   &
      ...
      real, intent(inout), dimension(ibmyj:iemyj,jbmyj:jemyj) :: mixht,akhs,akms,ct,snow,sice,thz0,qz0,uz0,vz0,u10e,v10e,tshltr,qshltr,pshltr,z0base,zntmyj
...
!-----------------------------------------------------------------------
      ...
      real :: z10tmp,wt_top,wt_bot
      double precision :: u10tmp,v10tmp,s10tmp,th10tmp,q10tmp
      integer :: k10idx_t,k10idx_b

    !$acc declare &
    !$acc present(xh,uh,ruh,xf,arf1,arf2,uf,vh,rvh,vf,zh,mh,rmh,c1,c2,zf,mf,rmf,   &
    !$acc        pi0,rho0,th0,rf0,rrf0,zs,gx,gy,xland,u0,v0)                      &
    !$acc present(tsk,znt,zntmp,ust,stau,thflux,qvflux,cd,ch,cq,u1,v1,s1,t1,q1, &
    !$acc      tst,qst,z0t,z0q,                                                   &
    ...
    !$acc      epst,epsd1,epsd2,thpten,upten,vpten,psfc,u10,v10,s10,th10,q10,hfx,qfx,hpbl, &
    ...
    dosfc:  IF( getsfc )THEN
      ...
      bbc3:  IF( bbc.eq.3 )THEN
        ...
        IF( use_avg_sfc .and. ( sfcmodel.eq.1 .or. sfcmodel.eq.5 ) )THEN
            call getavgsfc(u1,v1,s1,t1,q1,avgsfcu,avgsfcv,avgsfcs,avgsfct,avgsfcq)
        ENDIF

        IF( sfcmodel.eq.1 )THEN

          call getcecd(xh,u1,v1,s1,t1,q1,ppten(ib,jb,1),u10,v10,s10,th10,q10,xland,znt,tsk,qsfc,psfc,ust,tst,qst,z0t,z0q,cd,ch,cq,avgsfcu,avgsfcv,avgsfcs,avgsfct,avgsfcq)
          ...
          !For spray-laden HBL, use an interpolated u10, th10, q10
          if (testcase.eq.15) then

             !First find the z-index on either side of the 10-meter mark
             !$acc update self(zh)
             do k=1,nk
                z10tmp = zh(1,1,k)
                if (z10tmp .ge. 10.0) EXIT
             enddo
             k10idx_t = k
             k10idx_b = k-1
             wt_top = (10.0-zh(1,1,k10idx_b))/(zh(1,1,k10idx_t)-zh(1,1,k10idx_b))
             wt_bot = (zh(1,1,k10idx_t)-10.0)/(zh(1,1,k10idx_t)-zh(1,1,k10idx_b))

             !Now that we have the 10-meter index, interpolate
             !$acc parallel loop gang vector collapse(2) default(present) 
             do j=1,nj
             do i=1,ni
                u10(i,j) = ugr(i,j,k10idx_b)*wt_bot + ugr(i,j,k10idx_t)*wt_top
                v10(i,j) = vgr(i,j,k10idx_b)*wt_bot + vgr(i,j,k10idx_t)*wt_top
                s10(i,j) = sqrt(u10(i,j)**2 + v10(i,j)**2)
                th10(i,j) = (tha(i,j,k10idx_b)+th0(i,j,k10idx_b))*wt_bot + (tha(i,j,k10idx_t)+th0(i,j,k10idx_t))*wt_top
                q10(i,j) = qa(i,j,k10idx_b,nqv)*wt_bot + qa(i,j,k10idx_t,nqv)*wt_top
             enddo
             enddo

             !Now perform a 2D average
             call getavgsfc(u10,v10,s10,th10,q10,u10tmp,v10tmp,s10tmp,th10tmp,q10tmp)
             ...
             do i=1,ni
                ...
                th10(i,j) = th10tmp
                q10(i,j) = q10tmp
                !For stability purposes, only update this after the turbulence has spun up
                if (rtime .gt. 300.0) ust(i,j) = max( sqrt(cnstcd)*s10(i,j) , 1.0e-6 )
             enddo
             enddo
          end if
          ...
          if(isfcflx.eq.1)then
            call sfcflux(dt,ruh,xf,rvh,pi0s,ch,cq,pi0,thv0,th0,rf0,tsk,thflux,qvflux,mavail, &
                         ...
                         qbudget(8),psfc,u10,v10,s10,th10,q10,qsfc,znt,rtime)
          endif
...
!   DNS (constant viscosity)
      ...
      IF( cm1setup.eq.3 )THEN
        ...
        !$acc data copy(viscosity,pr_num,sc_num)
...
!--------------------------------------------------------------
    ...
    IF( tbc.eq.4 )THEN
      ...
      ! Richter and Wainwright (2023) upper boundary condition on winds:

      !$omp parallel do default(shared) private(i,j)
      !$acc parallel loop gang vector collapse(2) default(present)
      do j=1,nj+1
      do i=1,ni+1
        t13(i,j,nk+1) = t13(i,j,1)
        t23(i,j,nk+1) = t23(i,j,1)
      enddo
      enddo
