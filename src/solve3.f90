...
!-----------------------------------------------------------------------------
  ...
  CONTAINS
      ...
      use input, only : ib,ie,jb,je,kb,ke,itb,ite,jtb,jte,ktb,kte,ibb2,ibe2,jbb2,jbe2, &
          ...
          xc_uforce,xr_uforce,zr_uforce,alpha_uforce,t1_uforce,t2_uforce,iprcl,imove,sfcmodel, &
          nutk,nvtk,nwtk,cgs1,cgs2,cgs3,cgt1,cgt2,cgt3,rzt,zt,rdx,rdy,rdz,nqg,nqr,dot2p, &
          ...
          td_nudge,qd_nudge,prcl_droplet,ntwk,pdcomp,nbc,ebc,sbc,wbc,two_way_coupling, &
          output_rain,output_sws,output_svs,output_sps,output_srs,output_sgs,output_sus,output_shs, &
          humidity_control,humidity_target_rh,humidity_sink,humidity_gain,humidity_sink_min, &
          humidity_sink_max,humidity_control_elapse
      ...
      use droplet_module, only : droplet_driver
...
! Arrays and variables passed into solve
      ...
      real, intent(in), dimension(ib:ie,jb:je,kb:ke) :: rho0,prs0,thv0,rth0,qv0,qc0
      real, intent(inout), dimension(ib:ie,jb:je,kb:ke) :: pi0,th0
...
! Arrays and variables defined inside solve
      ...
      integer :: num_dropsub,i_dropsub
      real :: dt_dropsub
      double precision :: dbldt_dropsub,mtime_dropsub

      double precision, save :: humidity_last_update = -1.0d0
      double precision :: sumRH_local,sumRH_global,count_local,count_global
      double precision :: meanRH,meanRH_pct,new_sink
      real :: qvs_rh,t_rh,rh_cell
      logical :: humctrl_due
...
!Begin:  message passing
      ...
      IF( psolver.eq.2 .or. psolver.eq.3 .or. psolver.eq.6 .or. psolver.eq.7 )THEN
        ...
        elseif( psolver.eq.6 )then
          ...
          ! print *, 'Here 3 -- Not being executed'
          !$acc parallel loop gang vector collapse(3) default(present) private(i,j,k)
          !$omp parallel do default(shared)   &
          !$omp private(i,j,k)
          do k=1,nk
...
!  Equate the two arrays
      ...
      IF( dot2p )THEN
        rdt = 1.0/dt
        !print *, 'Here 4  -- Not being executed'
      ...
      if( do_lsnudge .and. donudge .and. do_lsnudge_u )then
        ...
        enddo
        !-----
        !print *, 'Here 5 -- Not being executed'
        !$acc parallel loop gang vector collapse(3) default(present) private(i,j,k)
        do k=1,nk
        do j=jb,je
        do i=ib,ie+1
...
!$omp private(i,j,k)
            ...
            IF(axisymm.eq.0)THEN
              !print *, 'Here 6 -- Not being executed'
              ! Cartesian grid:
              !$acc parallel loop gang vector collapse(3) default(present) private(i,j,k)
              do k=1,nk
              do j=1,nj+1
            ...
            ENDIF
            !JMD IF(k.gt.1)THEN
              !print *, 'Here 7 -- Not being executed'
              !$acc parallel loop gang vector collapse(3) default(present) private(i,j,k)
              do k=1,nk
              do j=1,nj
              do i=1,ni
        ...
        ELSE
          ! psolver=1,4,5:
            ...
            enddo
            IF(axisymm.eq.0)THEN
              print *, 'Here 9 -- Not being executed'
              ! Cartesian grid:
              !$acc parallel loop gang vector collapse(3) default(present) private(i,j,k)
              do k=1,nk
              do j=1,nj+1
            ...
            ENDIF
            IF(k.gt.1)THEN
              print *, 'Here 10 -- Not being executed'
              !$acc parallel loop gang vector collapse(3) default(present) private(i,j,k)
              do k=1,nk
              do j=1,nj
              do i=1,ni
...
!----------
      ...
      if(iptra.eq.1)then
          ...
          print *, 'Here 16 -- Not being executed'
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      IF(iprcl.eq.1)THEN
        ...
        if(terrain_flag)then
          ...
          print *,'Here 20 -- Not being executed'
          !$acc parallel loop gang vector collapse(3) default(present) private(i,j,k)
          !$omp parallel do default(shared)  &
          !$omp private(i,j,k)
          DO k=1,nk
          ...
          print *,'Here 21 -- Not being executed'
          !$omp parallel do default(shared)  &
          ...
          DO k=1,nk
            ...
            ELSE
              ...
              r1 = 0.5*r1
              r2 = 0.5*r2
              !$acc parallel loop gang vector collapse(2) default(present) private(i,j,r1,r2)
              do j=0,nj+1
              do i=0,ni+1
                rrw(i,j,k)=rrw(i,j,k)                              &
          ...
          print *,'Here 22 -- Not being executed'
        ...
        if (prcl_droplet.ge.1) then
          ...
          num_dropsub = 1
          mtime_dropsub = mtime
          do i_dropsub = 1,num_dropsub
          if (myid.eq.0) print *, 'DHR2: In dropsub loop, i_dropsub=',i_dropsub

          dt_dropsub = dt/real(num_dropsub)
          dbldt_dropsub = dbldt/dble(num_dropsub)
          ...
          call   droplet_driver(dt_dropsub,dbldt_dropsub,mtime_dropsub,xh,uh,ruh,xf,yh,vh,rvh,yf,zh,mh,rmh,zf,mf,zs, &
          ...
          IF( two_way_coupling.eq.1 )THEN
            if (testcase.eq.16) then
              !$acc update device(humidity_sink)
            endif
            !$omp parallel do default(shared) private(i,j,k)
            !$acc parallel loop gang vector collapse(3) default(present) private(i,j,k)
            do k=1,nk
            do j=1,nj+1
            do i=1,ni+1
              th3d(i,j,k)=th3d(i,j,k)+dt_dropsub*dpten(i,j,k,1)
              q3d(i,j,k,nqv)=q3d(i,j,k,nqv)+dt_dropsub*dpten(i,j,k,2)
              u3d(i,j,k)=u3d(i,j,k)+dt_dropsub*dpten(i,j,k,3)
              v3d(i,j,k)=v3d(i,j,k)+dt_dropsub*dpten(i,j,k,4)
              w3d(i,j,k)=w3d(i,j,k)+dt_dropsub*dpten(i,j,k,5)
          
	      !For Pi chamber DNS with periodic sidewalls, need a vapor sink
	      ...
	      if (testcase.eq.16) then
                 ...
                 q3d(i,j,k,nqv)=max(0.0,q3d(i,j,k,nqv)-dt_dropsub*humidity_sink)
	      end if
            enddo
            enddo
            enddo

            if (humidity_control.eq.1 .and. testcase.eq.16 .and. imoist.eq.1) then
              humctrl_due = .false.
              if (humidity_control_elapse.le.0.0) then
                humctrl_due = .true.
              elseif (humidity_last_update.lt.0.0d0) then
                humctrl_due = .true.
              elseif ( (mtime_dropsub-humidity_last_update) .ge. dble(humidity_control_elapse) ) then
                humctrl_due = .true.
              endif

              if (humctrl_due) then
                sumRH_local = 0.0d0
                count_local = dble(ni)*dble(nj)*dble(nk)
                !$acc parallel loop gang vector collapse(3) default(present) private(i,j,k,qvs_rh,t_rh,rh_cell) reduction(+:sumRH_local)
                do k=1,nk
                do j=1,nj
                do i=1,ni
                  t_rh = (th0(i,j,k)+th3d(i,j,k))*(pi0(i,j,k)+ppi(i,j,k))
                  qvs_rh = rslf( prs(i,j,k) , t_rh )
                  rh_cell = q3d(i,j,k,nqv)*(1.0+qvs_rh*reps) / (max(1.0e-12,qvs_rh)*(1.0+q3d(i,j,k,nqv)*reps))
                  sumRH_local = sumRH_local + dble(rh_cell)
                enddo
                enddo
                enddo

                sumRH_global = sumRH_local
                count_global = count_local

                meanRH = sumRH_global/max(1.0d0,count_global)
                meanRH_pct = 100.0d0*meanRH

                new_sink = dble(humidity_sink) + dble(humidity_gain)*(dble(humidity_target_rh)-meanRH_pct)
                new_sink = min(dble(humidity_sink_max),max(dble(humidity_sink_min),new_sink))
                humidity_sink = sngl(new_sink)
                !$acc update device(humidity_sink)
                humidity_last_update = mtime_dropsub

                if (myid.eq.0) then
                  write(*,'(a,f12.4,a,f10.4,a,es12.4)') 'HUMCTRL time=',mtime_dropsub,' meanRH(%)=',meanRH_pct,' sink=',humidity_sink
                endif
              endif
            endif
          ...
          mtime_dropsub = mtime_dropsub + dbldt_dropsub
          enddo ! droplet sub-loop         

        else  ifdroplet
          stop 'solve3: before call to parcel_driver'
...
!--------------------------------------------------------------------
    ...
    IF( output_rain.eq.1 )THEN
      ...
      if(imove.eq.999.and.imoist.eq.1)then
        weps = 10.0*epsilon
...
!$acc parallel loop gang vector collapse(2) default(present) private(i,j,tem,n)
!$omp parallel do default(shared)  &
...
print *,'Here 25 -- Not being executed'
!$acc parallel loop gang vector default(present) private(j,k,i,r1,r2)
!$omp parallel do default(shared)   &
!$omp private(i,j,k,r1,r2)
      do j=0,nj+2
        ! lowest model level:
      ...
      enddo
      k = 1
print *,'Here 26 -- Not being executed'
!$acc parallel loop gang vector collapse(2) default(present) private(i,j,n,r1,tem)
!$omp parallel do default(shared)  &
...
!$acc loop private(n)
  ...
  ELSE
      ! Axisymmetric grid:
print *,'Here 27 -- Not being executed'
!$acc parallel loop gang vector collapse(2) default(present) private(i,j,n,tem)
!$omp parallel do default(shared)  &
!$omp private(i,j,n,tem)
      do j=1,nj+1
      do i=1,ni+1
...
!$omp private(i,j,k)
        ...
        ! get height AGL:
print *,'Here 28 -- Not being executed'
!$acc parallel loop gang vector collapse(3) default(present) private(i,j,k)
!$omp parallel do default(shared)  &
!$omp private(i,j,k)
        do k=1,nk+1
...
!$acc parallel loop gang vector collapse(2) default(present) private(i,j,k,n,tem)
!$omp parallel do default(shared)  &
...
!$acc parallel loop gang vector collapse(3) default(present) private(i,j,n)
!$omp parallel do default(shared)  &
!$omp private(i,j,n)
      do j=1,nj
      do i=1,ni
