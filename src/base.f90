...
!
      ...
      ELSEIF( isnd.eq.7 .or. isnd.eq.17 )then
        ...
        ! GHB, 220302: initialize all arrays
...
!  Dry, constant d(theta)/dz sounding
      ...
      ELSEIF(isnd.eq.8)THEN
        ...
        ! GHB, 220302: settings for ASD project
        ! (based on "zrall/cavg80.dat" dropsonde composite)
        th_sfc     =     302.0
        prs_sfc    =    94500.0
        lapse      =    0.006
        ...
        ! NOTE: sea-surface temperature (tsk) is over-written here.
        ! thus, value in namelist.input does not matter

        tsk0  =  t_sfc +  var4  ! sst is xxx K warmer than t_sfc
                                ! (after spin-up, results in ~2 K sea-air temp difference)
...
!  Reference:  Bryan and Rotunno, 2009, JAS, v10, pp. 3042-3060
      ...
      ELSEIF(isnd.eq.11)THEN
        ...
        ttype =  1    ! 1 = reversible
                      ! 2 = pseudoadiabatic

        !psurf = 101510.0    ! surface pressure (Pa)
        psurf = 83230.0    ! surface pressure (Pa)
...
!  fill in ghost cells
      ...
      call bcs(pi0, device=.false.)
      call bcs(prs0,device=.false.)
      call bcs(th0, device=.false.)
      call bcs(qv0, device=.false.)
      call bcs(qc0, device=.false.)
      call bcs(qi0, device=.false.)
      call bcs(rh0, device=.false.)
       



      call extrapbcs(pi0,device=.false.)
      call extrapbcs(prs0,device=.false.)
      call extrapbcs(th0,device=.false.)
      call extrapbcs(qv0,device=.false.)
      call extrapbcs(qc0,device=.false.)
      call extrapbcs(qi0,device=.false.)
      call extrapbcs(rh0,device=.false.)
...
!!!        dvdr(k) = -0.00085*max(0.0,1.0-zh(1,1,k)/18000.0)
        ...
        dvdr(k) = -var3*(v0(1,1,k)+vmove)/var2
