  ...
  integer :: ngidx  !Each 1 rank will keep a "global" particle index counter which increases for every new particle created on this rank
  ...
  CONTAINS
      ...
      subroutine droplet_driver (dt,dbldt,mtime,xh,uh,ruh,xf,yh,vh, &
                                 ...
                                 reqs_s,dpten,pdata_locind,nstep)
      ...
      use input, only : ib,ie,jb,je,kb,ke,ibl,iel,jbl,jel,numq,ni,nj, &
          ...
          prms,przs,pru,prv,prw,prt,prqv,prprs,prrho,prtime,prpidx,prprocidx, &
          ...
          time_dropC4,ngxy,dx,dy,dz,testcase,drop_traj
      use constants
      use comm_module
...
!-----------------------------------------------------------------------
      ...
      double precision, intent(inout), dimension(ib:ie,jb:je,kb:ke,7) :: dpten
...
!!!      real, dimension(ib:ie,jb:je,kb:ke) :: ta   ! GHB: now passed from solve3
      ...
      integer :: Arrive(num_nn)                            ! Number of droplets that will enter the current
                                                           ! 1 region from each nearest neighbor
      ...
      integer, dimension(nparcelsLocal) :: pdata_neighbor  ! array to store the new 1 region info for all the droplets
      integer(i8) :: np_tmp
      integer :: neighbor                                  ! indicate which 1 region a droplet stays after this time step 
      ...
      !    deterministic but varies between different 1 ranks and time steps
      ...
      !    more randomness but unmatched results between initial and restart runs
        ...
        particle_seed =  (myid+values(8)+values(7)+values(6))
...
! JS: comment out this conflict check for the new 1 interface;
...
!  note Step 1 is also needed for a non-1 run

     ! JS: send/receive the droplet information through 1;
    ...
    !         the current 1 region with its nearest neighbor      !
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    !         enter/leave the current 1 region with its nearest neighbor !
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! Most of the indirect address arrays are calculated on the CPU.
    ! So copy them to the GPU

    ! On the GPU, move droplets between different 1 ranks.
...
! Requires inputting the current 10-meter wind speed
   if ( .not. use_wall_bc ) then  
      u10_ssgf = 0.0
      ! If 1 macro is enabled than perform the following sum on
      ! the CPU 
      !$acc update host(s10)
      do j=1,nj
         do i=1,ni
      ...
      if (myid==0) then 
         ...
         write(*,'(a18,i)')  'Total particles:',np_tmp
      end if
...
!----------------------------------------------------------------------
   ...
   subroutine BE_integration(dt,dbldt,np,pdata,pdata_locind,xf,yf, &
      ...
      use input, only : ib,ie,jb,je,kb,ke,numq,npvals,nx,ny,viscosity, &
          ...
          prvpx,prvpy,prvpz,prrp,prrp0,prms,prtp,prx,pry,prsig,prz,pru,prv, &
          prw,prt,prqv,prprs,prrho,pract,prtime,prmult,ni,nj,mywest, &
          mysw,mynw,myeast,myse,myne,mysouth,mynorth,nparcelsLocal, &
          myid,dx,dy,pi_sp,pi_dp,ngxy,nbc,sbc,ebc,wbc,tsk0,drop_evap
      use constants , only : rhow,rhow_dp,undefined_index,cpl,cpl_dp,cp_dp
      use comm_module
      use parcel_module
      use mpi
      ...
      integer, intent(out) :: neighbor  ! indicate which 1 region a droplet 
...
!!!      volp0 = 4.0/3.0*pi_sp*rp0**3    ! not used
      ...
      if (pdata(np,pract).lt.0.0) then
         ...
         ! determine which new 1 region this droplet 
...
!  Project to scalar points
      ...
      ! diagnostic purposes only: number and liquid mixint ratio of drops in each grid cell
      !$acc atomic update
      dpten(iflag,jflag,kflag,6) = dpten(iflag,jflag,kflag,6) + 1.0

      !$acc atomic update
      dpten(iflag,jflag,kflag,7) = dpten(iflag,jflag,kflag,7) + partmass/rhoval*pdata(np,prmult)/dV
      ...
      double precision, intent(in), dimension(ib:ie,jb:je,kb:ke,7) :: dpten
      real, intent(in), dimension(nparcelsLocal,npvals) :: pdata
      integer, intent(inout), dimension(nparcelsLocal,3) :: pdata_locind    ! x/y/z location index of each parcel
      real, intent(in), dimension(ib:ie,jb:je,kb:ke) :: zh
      real, intent(in), dimension(ib:ie,jb:je,kb:ke+1) :: zf
      ...
      real :: partnum(nk),multnum(nk),partmass(nk),numconc(nk),massconc(nk),vp1mean(nk),vp2mean(nk),vp3mean(nk),vp1msqr(nk),vp2msqr(nk),vp3msqr(nk),uf1mean(nk),uf2mean(nk),uf3mean(nk),Tpsrc(nk),qvsrc(nk),rhovsrc(nk),qf(nk),Tf(nk),radmean(nk),radmsqr(nk),Tpmean(nk),Tpmsqr(nk),qfsat(nk),m1src(nk),m2src(nk),m3src(nk),qstarmean(nk),kmoist(nk),mse(nk),wk(nk),wtmp(nk)
      ...
      integer, parameter :: num0 = 33, num1 = 30   !Number of 0th and 1st order statistics in droplet_diag
      real :: rad10_tmp
      double precision :: qstar_tmp
      integer :: ibin

      double precision, dimension(num0) :: droplet_diag0     ! temporary array for 1 reduce operation for the 0th-order diagnostic except rtime
      double precision, dimension(nk,num1) :: droplet_diag1     ! temporary array for 1 reduce operation for the 0th-order diagnostic except rtime
      ...
      droplet_diag0(30) = radmin
      droplet_diag0(31) = Tpmin
      droplet_diag0(32) = radmax
      droplet_diag0(33) = Tpmax

      ! The root rank will collect the diagnostic from different 1 ranks
      if ( myid == 0 ) then
         call mpi_reduce(MPI_IN_PLACE,droplet_diag0(1),num0-4,MPI_DOUBLE_PRECISION, &
                         MPI_SUM,0,MPI_COMM_WORLD,ierr)
      else
         call mpi_reduce(droplet_diag0(1),droplet_diag0(1),num0-4,MPI_DOUBLE_PRECISION, &
                         MPI_SUM,0,MPI_COMM_WORLD,ierr)
      end if
      if ( myid == 0 ) then
         call mpi_reduce(MPI_IN_PLACE,droplet_diag0(num0-3),2,MPI_DOUBLE_PRECISION, &
                         MPI_MIN,0,MPI_COMM_WORLD,ierr)
      else
         call mpi_reduce(droplet_diag0(num0-3),droplet_diag0(num0-3),2,MPI_DOUBLE_PRECISION, &
                         MPI_MIN,0,MPI_COMM_WORLD,ierr)
      end if
      if ( myid == 0 ) then
         call mpi_reduce(MPI_IN_PLACE,droplet_diag0(num0-1),2,MPI_DOUBLE_PRECISION, &
                         MPI_MAX,0,MPI_COMM_WORLD,ierr)
      else
         call mpi_reduce(droplet_diag0(num0-1),droplet_diag0(num0-1),2,MPI_DOUBLE_PRECISION, &
                         MPI_MAX,0,MPI_COMM_WORLD,ierr)
      end if
      if ( myid == 0 ) then 
         ...
         radmin = droplet_diag0(30)
         Tpmin = droplet_diag0(31)
         radmax = droplet_diag0(32)
         Tpmax = droplet_diag0(33)
      ...
      do k=1,nk
         ...
         do j=1,nj
            do i=1,ni
               ...
               qt = qa(i,j,k,nqv) + dpten(i,j,k,7)/rho(i,j,k)  !Total mixing ratio of vapor + liquid
      ...
      if ( myid == 0 ) then 
         ...
         !Assign back to the arrays, now summed across 1 ranks
         do iz=1,nk
      ...
      !$acc update host (partnumrz,multnumrz,vp3meanrz)
       ...
       !Sum up the histograms across 1 processes
       if (myid == 0) then
	 ...
	 tnumpart_r(ibin) = tnumpart_r(ibin) + 1.0
      ...
      !$acc update host (hist_radius,hist_radius10,hist_restime,hist_fallrad,tnumpart_r)
      ...
      ! JS: this subroutine is not updated with the new 1
      ...
      call writedropdiag_nc(nrec,nbins, &
           ...
           dumzh,numconc,massconc,vp1mean,vp2mean,vp3mean,uf1mean,uf2mean,uf3mean,vp1msqr,vp2msqr,vp3msqr,m1src,m2src,m3src,Tpsrc,qvsrc,rhovsrc,Tf,qf,Tpmean,Tpmsqr,radmean,radmsqr,qfsat,qstarmean,kmoist,mse,wk,multnumrz,vp3meanrz,tnumpart_r)
      end if
...
200     format(2x,'DROPLET_DIAG:: ',A10,':',1x,e13.6)
      ...
      real, intent(in), dimension(nparcelsLocal,npvals) :: pdata
      integer, parameter :: num_local_traj = 500 !The maximum number of droplets which could be selected on a single 1 rank
      ...
      if ( loc_ind .ne. undefined_index ) then
         ...
         if ( ind .lt. 0 ) then
           ...
           !    - a parcel enters a new or different 1 process
         i = end_ind
         do while( ind .lt. 0 .and. i .gt. 1 )
            i = i - 1
            if ( loc .ge. dsize(i) .and. loc .le. dsize(i+1) ) then
      ...
      use input, only : nparcelsLocal,npvals,myid,ni,nj,ib,ie,jb,je, &
                        prx,pry,prz,pru,prv,prw,prvpx,prvpy,prvpz,prrp,prrp0,prtp,prms,pract,prtime, &
  ...
  !   ===== update x-axis for each bin =====
      ...
      if (myid==0) write(*,'(a15,i,9e15.6)')  'DROP INJECTION:',my_reintro,my_reintro_tmp,drop_mult,u10,totdrops,xrange,yrange,num_injected,mtime,time_since_inject
        ...
        particle_seed =  (myid+values(8)+values(7)+values(6))
         ...
         do j=1,my_reintro
            ...
            if (random_droplets) then
               ...
               !particle_seed = floor(29.*mtime) + (97*myid) + (7*j)
               call new_RandomNumberSequence(randomNumbers, j*particle_seed)
    ...
    if (u10 .ge. 4.0 .and. u10 .lt. 11.0) then
       cdn10 = 1.2d-3
    ...
    subroutine droplet_injection_pi(pdata,xf,yf,time_since_inject,mtime,my_reintro)
      ...
      if (myid==0) write(*,'(a15,i,8e15.6)')  'DROP INJECTION:',my_reintro,my_reintro_tmp,drop_mult,xrange,yrange,zrange,num_injected,mtime,time_since_inject
    ...
    !$acc               num100,num1000)
      ...
      subroutine writedropdiag_nc(nrec,nbins,&
                                  rtime,tnumpart,tnummult,tnumaerosol,tnumdrop,tmasspart,radavg,radsqr,radmin,radmax,Tpavg,Tpsqr,Tpmin,Tpmax,Tfavg,qfavg,prsavg,rhoavg,&
                                  zhvec,numconc,massconc,vp1mean,vp2mean,vp3mean,uf1mean,uf2mean,uf3mean,vp1msqr,vp2msqr,vp3msqr,m1src,m2src,m3src,Tpsrc,qvsrc,rhovsrc,Tf,qf,Tpmean,Tpmsqr,radmean,radmsqr,qfsat,qstarmean,kmoist,mse,wk,multnumrz,vp3meanrz,tnumpart_r)
      use input
      use constants
      use netcdf
      ...
      real,    intent(in)    :: zhvec(nk),numconc(nk),massconc(nk),vp1mean(nk),vp2mean(nk),vp3mean(nk),vp1msqr(nk),vp2msqr(nk),vp3msqr(nk),m1src(nk),m2src(nk),m3src(nk),Tpsrc(nk),qvsrc(nk),rhovsrc(nk),Tf(nk),qf(nk),Tpmean(nk),Tpmsqr(nk),radmean(nk),radmsqr(nk),qfsat(nk),qstarmean(nk),uf1mean(nk),uf2mean(nk),uf3mean(nk),kmoist(nk),mse(nk),wk(nk)
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      ! This subroutine will check if a droplet falls into this 1 region or not
      !     Exception case:
      ...
      !         let the 1 rank which inclues that boundary owns it
      !
