...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      subroutine turbs(iflux,dt,dosfcflx,xh,rxh,arh1,arh2,uh,xf,arf1,arf2,uf,vh,vf,sflux,  &
                       ...
                       dobud,ibd,ied,jbd,jed,kbd,ked,ndiag,diag,sd_hturb,sd_vturb,ivar,pi0,prs0)
      use input, only: ib,ie,jb,je,kb,ke,itb,ite,jtb,jte,ktb,kte,ibc,iec,jbc,jec, &
          kbc,kec,ni,nj,nk,rdx,rdy,rdz,wbc,ebc,nbc,sbc,ibw,ibe,ibs,ibn,sc_num,pr_num,cm1setup, &
          cgs1,cgs2,cgs3,cgt1,cgt2,cgt3,viscosity,axisymm,timestats,time_ttend,mytime,bc_temp, &
          ptc_bot,ptc_top,dohturb,dovturb,base_pbot,base_pibot,base_ptop,base_pitop,doimpl, &
          bcturbs,terrain_flag,ptc_n,ptc_s,ptc_w,ptc_e
      use constants
      ...
      real, intent(in), dimension(ib:ie,jb:je,kb:ke) :: pi0,prs0
      ...
      real :: ptc_n_tmp,ptc_s_tmp,ptc_w_tmp,ptc_e_tmp
      ...
      !$acc present(sten,pi0,prs0) &
      !$acc present(rho,rr,rf,s,khh,khv,dum7,dum8)

!---------------------------------------------------------------

!!  !$acc update device (ivar)
  ...
  if( cm1setup.eq.3 )then
      ...
      if (ivar.eq.2) then      !! for DNS of moist RB convection
  ...
  IF(.not.terrain_flag)THEN
    ...
    IF(axisymm.eq.0)THEN
      ...
      !Lateral Dirichlet BCs for scalars using DNS
      !ptc_s,n,e,w are assumed to be temperature, not potential temperature!
      IF (cm1setup.eq.3) THEN
         IF (wbc.eq.4 .and. ibw.eq.1) THEN
            !$omp parallel do default(shared) private(i,k)
            !$acc parallel loop gang vector collapse(2) default(present) private(j,k)
            do k=1,nk
            do j=1,nj
              if (ivar.eq.1) then
                ptc_w_tmp = ptc_w/pi0(1,j,k) - th0r
              elseif (ivar.eq.2) then
                ptc_w_tmp = rslf(prs0(1,j,k),ptc_w)
              endif
              dumx(1,j,k) = -kappa*2.0*(s(1,j,k)-ptc_w_tmp)*rdx*(rho(1,j,k)+rho(2,j,k))*uf(1)*0.5
            enddo
            enddo
         ENDIF

         IF (ebc.eq.4 .and. ibe.eq.1) THEN
            !$omp parallel do default(shared) private(i,k)
            !$acc parallel loop gang vector collapse(2) default(present) private(j,k)
            do k=1,nk
            do j=1,nj
              if (ivar.eq.1) then
                ptc_e_tmp = ptc_e/pi0(ni,j,k) - th0r
              elseif (ivar.eq.2) then
                ptc_e_tmp = rslf(prs0(ni,j,k),ptc_e)
              endif
              dumx(ni+1,j,k) = kappa*2.0*(s(ni,j,k)-ptc_e_tmp)*rdx*(rho(ni,j,k)+rho(ni-1,j,k))*uf(ni)*0.5
            enddo
            enddo
         ENDIF

         IF (sbc.eq.4 .and. ibs.eq.1) THEN
            !$omp parallel do default(shared) private(i,k)
            !$acc parallel loop gang vector collapse(2) default(present) private(i,k)
            do k=1,nk
            do i=1,ni
              if (ivar.eq.1) then
                ptc_s_tmp = ptc_s/pi0(i,1,k) - th0r
              elseif (ivar.eq.2) then
                ptc_s_tmp = rslf(prs0(i,1,k),ptc_s)
              endif
              dumy(i,1,k) = -kappa*2.0*(s(i,1,k)-ptc_s_tmp)*rdy*(rho(i,1,k)+rho(i,2,k))*vf(1)*0.5
            enddo
            enddo
         ENDIF

         IF (nbc.eq.4 .and. ibn.eq.1) THEN
            !$omp parallel do default(shared) private(i,k)
            !$acc parallel loop gang vector collapse(2) default(present) private(i,k)
            do k=1,nk
            do i=1,ni
              if (ivar.eq.1) then
                ptc_n_tmp = ptc_n/pi0(i,nj,k) - th0r
              elseif (ivar.eq.2) then
                ptc_n_tmp = rslf(prs0(i,nj,k),ptc_n)
              endif
              dumy(i,nj+1,k) = kappa*2.0*(s(i,nj,k)-ptc_n_tmp)*rdy*(rho(i,nj,k)+rho(i,nj-1,k))*vf(nj)*0.5
            enddo
            enddo
         ENDIF
      ...
      IF( cm1setup.eq.1 .or. cm1setup.eq.2 )THEN
        ...
        if(iflux.eq.1 .and. dosfcflx)then
          ...
          if( bcturbs.eq.3 )then
            ...
            ! Richter and Wainwright (2023) upper boundary condition:

            !$omp parallel do default(shared) private(i,j)
            !$acc parallel loop gang vector collapse(2) default(present)
            DO j=1,nj
              do i=1,ni
                dumz(i,j,nk+1) = sflux(i,j)*rf(i,j,nk+1)
              enddo
            ENDDO
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      use input, only : ib,ie,jb,je,kb,ke,itb,ite,jtb,jte,ibt,iet,jbt, &
          jet,kbt,ket,ibc,iec,jbc,jec,kbc,kec,ni,nj,nk,rdx,rdy,rdz, &
          wbc,ebc,nbc,sbc,ibw,ibe,ibn,ibs,zt,rzt,doimpl,timestats,time_ttend, &
          mytime,terrain_flag
      use constants
      implicit none
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      use input, only : ib,ie,jb,je,kb,ke,itb,ite,jtb,jte,ktb,kte,ibc,iec, &
          jbc,jec,kbc,kec,ib2pt,ie2pt,jb2pt,je2pt,kb2pt,ke2pt,ibdv,iedv, &
          jbdv,jedv,kbdv,kedv,nudiag,ni,nj,nk,rdx,rdy,rdz,wbc,ebc,nbc,sbc, &
          ibw,ibe,ibs,ibn,cgs1,cgs2,cgs3,cgt1,cgt2,cgt3,zt,dovturb,dohturb, &
          axisymm,doimpl,dot2p,ntwk,timestats,time_ttend,mytime,terrain_flag, &
          ud_hturb,ud_vturb
      use constants
      implicit none
      ...
      IF( dot2p )THEN
        ...
        do k=1,ntwk
          ...
          do i=i1,i2
            ...
            ! 220308: save the 2-pt tendency
            rru0 = rdz*mh(1,1,k)/(0.5*(rho(i-1,j,k)+rho(i,j,k)))
            u2pt(i,j,k)=(dum2(i,j,k+1)-dum2(i,j,k))*rru0
          enddo
          enddo
        enddo
...
! 220308: GHB, move this code; see t2p section above
!      if( dot2p )then
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      use input, only : ib,ie,jb,je,kb,ke,itb,ite,jtb,jte,ktb,kte,ibc,iec, &
          jbc,jec,kbc,kec,ib2pt,ie2pt,jb2pt,je2pt,kb2pt,ke2pt,ibdv,iedv, &
          jbdv,jedv,kbdv,kedv,nvdiag,ni,nj,nk,rdx,rdy,rdz,wbc,ebc,nbc,sbc, &
          ibw,ibe,ibs,ibn,cgs1,cgs2,cgs3,cgt1,cgt2,cgt3,zt,dovturb,dohturb, &
          axisymm,doimpl,dot2p,ntwk,timestats,time_ttend,mytime,terrain_flag, &
          vd_vturb,vd_hturb
      use constants
      ...
      IF( dot2p )THEN
        ...
        do k=1,ntwk
          ...
          do i=1,ni
            turbz(i,j,k) = turbz(i,j,k)+rdz*mh(1,1,k)*(dum2(i,j,k+1)-dum2(i,j,k))
            ! 220308: save the 2-pt tendency
            rrv0 = rdz*mh(1,1,k)/(0.5*(rho(i,j-1,k)+rho(i,j,k)))
            v2pt(i,j,k)=(dum2(i,j,k+1)-dum2(i,j,k))*rrv0
          enddo
          enddo
        enddo
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      use input, only : ib,ie,jb,je,kb,ke,itb,ite,jtb,jte,ktb,kte,ibc,iec, &
          jbc,jec,kbc,kec,ib2pt,ie2pt,jb2pt,je2pt,kb2pt,ke2pt,ibdv,iedv, &
          jbdv,jedv,kbdv,kedv,nwdiag,ni,nj,nk,rdx,rdy,rdz,wbc,ebc,nbc,sbc, &
          ibw,ibe,ibs,ibn,cgs1,cgs2,cgs3,cgt1,cgt2,cgt3,zt,dovturb,dohturb, &
          axisymm,doimpl,dot2p,ntwk,timestats,time_ttend,mytime,terrain_flag, &
          wd_vturb,wd_hturb
      use constants
