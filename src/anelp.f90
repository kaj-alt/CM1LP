...
!---------------------------------------------------------------------
        ...
        if(irbc.eq.2)then
          if(ibw.eq.1 .or. ibe.eq.1) call radbcew(radbcw,radbce,ua,device=.false.)
 
          if(ibs.eq.1 .or. ibn.eq.1) call radbcns(radbcs,radbcn,va,device=.false.)
        endif
...
!  Open boundary conditions:
        ...
        IF( (wbc.eq.2.and.ibw.eq.1) .or. (ebc.eq.2.and.ibe.eq.1) )THEN
          !JMD FIXME: note that this operator assumes GPU resident memory 
