#!/bin/bash
# Build cm1lp_dh_ai (codex-pi) on Derecho — NVHPC + OpenACC (GPU) + MPI + NetCDF.
set -e
...
# Ensure mpif90 override is active (CRC builds switch this to mpiifx)
sed -i 's/override FC  = mp[^ ]*/override FC  = mpif90/' src/Makefile
